/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export function derive_signing_key_from_signature(a: number, b: number): number;
export function get_key_hierarchy_shares(a: number, b: number, c: number): void;
export function get_key_hierarchy(a: number, b: number): number;
export function get_managing_cluster_shares(a: number, b: number, c: number): void;
export function hex_to_field_scalar(a: number, b: number): number;
export function bigint_to_limbs(a: number, b: number): number;
export function add(a: number, b: number, c: number, d: number): number;
export function subtract(a: number, b: number, c: number, d: number): number;
export function compute_poseidon_hash(a: number, b: number): number;
export function generate_wallet_update_signature(a: number, b: number, c: number, d: number): number;
export function generate_external_transfer_signature(a: number, b: number, c: number, d: number): number;
export function sign_http_request(a: number, b: number, c: number, d: number, e: number, f: number): void;
export function sign_message(a: number, b: number, c: number, d: number): number;
export function __wbindgen_malloc(a: number, b: number): number;
export function __wbindgen_add_to_stack_pointer(a: number): number;
export function __wbindgen_realloc(a: number, b: number, c: number, d: number): number;
export function __wbindgen_free(a: number, b: number, c: number): void;
